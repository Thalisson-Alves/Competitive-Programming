...
void solve() {
  ...
  cin >> n >> q;
  vector<int> hist(n);
  auto add = [&](int l, int r, int c) { hist[c] += r-l+1; };
  auto rem = [&](int l, int r, int c) { hist[c] -= r-l+1; };
  auto upd = [&](int, int r, int pr, int c) { hist[c] += r-pr; };
  BlockSet<int, int> bl(add, rem, upd);
  for (int i = 0; i < n; i++) bl.add_unchecked(i, i, i);
  while (q--) {
    int t;
    cin >> t;
    if (t == 1) {
