...
struct SuffixAutomaton {
  ...
  int count(const string &t) const {
    return find(t).value_or(state{.cnt = 0}).cnt;
  }

  int first_pos(const string &t) const {
    return find(t).value_or(state{.first_pos = (int)t.size() - 2}).first_pos - (int)t.size() + 1;
  }
};
