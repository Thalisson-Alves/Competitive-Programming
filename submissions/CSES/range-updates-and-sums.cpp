...
struct L {
  ...
  friend ostream &operator<<(ostream &os, L o) {
    return os << "{set_value: " << o.set_value << ", acc: " << o.acc << ", is_set: " << o.is_set << "}";
  }
};
 
ll mapping(L a, ll x, int l, int r) {
  if (a.is_set)
    x = (r - l + 1) * a.set_value;
 
  return x + (r - l + 1) * a.acc;
}
 
L compose(L a, L b) {
  if (a.is_set) return a;
  b.acc += a.acc;
  return b;
}
...
void solve()
{
  int n, q;
  cin >> n >> q;
  SegTreeLazy<ll, F(a+b), L, mapping, compose> st(n);
  for (int i = 0; i < n; i++) {
    ll x;
    cin >> x;
    st.set(i, x);
  ...
  while (q--) {
    ...
    if (t == 1) {
      ll x;
      cin >> x;
      st.update(a, b, L(0, x, false));
    } else if (t == 2) {
      ll x;
      cin >> x;
      st.update(a, b, L(x, 0, true));
    } else {
      cout << st.query(a, b) << '\n';
    }
  }
