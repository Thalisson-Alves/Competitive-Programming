...
void topological_order(const vector<vector<int>> &g, vector<char> &vis,
...
void mark_component(const vector<vector<int>> &gr, vector<int> &component,
                    int s) {
  for (auto x : gr[s])
    if (component[x] == -1) {
...
vector<int> mark_components(const vector<vector<int>> &gr,
...
vector<set<int>> components_tree(const vector<vector<int>> &g,
...
void solve() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    --u, --v;
    if (w)
      sat.add_xor(u, false, v, false);
    else
      sat.add_or(u, false, v, false);
  }

  cout << (sat.solve().size() ? "YES" : "NO") << '\n';
}
