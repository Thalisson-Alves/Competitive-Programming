...
void solve() {
  ...
  while (q--) {
    ...
    hld.for_each_vertex(c, d, [&](int l, int r) {
      s2.emplace_back(l, r);
    });
    sort(s1.begin(), s1.end(), [](const auto &x, const auto &y) { return x.second < y.second; });
    sort(s2.begin(), s2.end(), [](const auto &x, const auto &y) { return x.second < y.second; });
    int res = 0;
    for (auto it = s1.begin(), jt = s2.begin(); it != s1.end() and jt != s2.end(); (it->second < jt->second ? ++it : ++jt)) {
      if (it->first <= jt->second and jt->second <= it->second)
        res += jt->second - max(it->first, jt->first) + 1;
      else if (jt->first <= it->second and it->second <= jt->second)
        res += it->second - max(it->first, jt->first) + 1;
    }
    cout << res << '\n';
  }
}
