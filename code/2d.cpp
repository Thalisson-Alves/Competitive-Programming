...
template<typename T=double> struct LineSegment {
  Point<T> a, b;

  LineSegment() : a(), b() {}
  LineSegment(Point<T> a_, Point<T> b_) : a(a_), b(b_) {}

  inline bool operator==(const LineSegment &l) const { return a == l.a and b == l.b; }

  inline bool intersects(const LineSegment &l) const {
  ...
  inline Point<T> intersection(const LineSegment &l) const {
    assert(intersects(l));
  ...
  friend ostream& operator<<(ostream &os, const LineSegment &l) { return os << l.a << ' ' << l.b; }
  friend istream& operator>>(istream &is, LineSegment &l) { return is >> l.a >> l.b; }
};
