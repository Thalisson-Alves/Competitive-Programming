struct SuccessorGraph {
  ...
  SuccessorGraph(const vector<int> &v) : path_num(v.size()), pos(v.size()) {
  ...
  int reach(int cur, int dest) const {
    int moves = 0;
    while (not is_cycle[path_num[cur]] and path_num[cur] != path_num[dest]) {
      auto &p = path(cur);
      moves += (int)p.size() - pos[cur] - 1;
      cur = p.back();
    }

    if (path_num[cur] != path_num[dest])
      return -1;

    if (pos[cur] <= pos[dest])
      return moves + pos[dest] - pos[cur];

    if (not is_cycle[path_num[cur]])
      return -1;

    return moves + pos[dest] + (int)path(cur).size() - pos[cur];
  }
