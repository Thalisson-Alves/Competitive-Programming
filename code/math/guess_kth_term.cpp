...
template <typename T> T get_kth(const vector<T> &rec, const vector<T> &dp, ll k) {
  int n = (int)rec.size();
...
template <typename T> T guess_kth_term(const vector<T> &s, ll k) {
