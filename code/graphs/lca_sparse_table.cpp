struct LCA {
  ...
  static pair<int,int> min(pair<int,int> a, pair<int,int> b) { return std::min(a, b); }
  SparseTable<pair<int, int>> table;
  LCA(vector<vector<int>> &g, int root = 0) : height(g.size()), euler(g.size() * 2), first(g.size()), table(min,{INT_MAX,INT_MAX}) {
